...
void cBsp::setPcbMake(float Make)
{
  ...
  if(pcbMake >= 2.0)
  {
    ...
    debugPrintln(digitalRead(hooterPin));
  }
...
/*Write pin on board*/
void cBsp::BSP_IOwtire(int Pin, int st)
{
    digitalWrite(Pin, st);
}
/****************************************************************
...
/*********************************
* Function BSP_DISPInit
**********************************/
void cBsp::displayInit(cMYDISPLAY *Disp)
{
    Disp->init(DISP_DATA, DISP_CLK, DISP_LATCH);
    debugPrintln("DISPLAY Init Done...");
}
/*********************************
* Read board power through opto
**********************************/
// uint8_t cBsp::BSP_getBoardPowerStatus(void)
// {
//     return !(BSP_IOread(BSP_PWR_DET));
// }
